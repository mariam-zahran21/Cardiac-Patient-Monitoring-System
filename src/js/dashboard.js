...
function renderCalendar(appointments, patients) {
    ...
    for (let day = 1; day <= totalDays; day++) {
        ...
        if (dayApps.length > 0) {
            // Highlight date with blue background/glow
            cell.className += "bg-teal-50 text-teal-700 hover:bg-teal-100 border border-teal-200/50 ";
            
            // Add a small blue dot under the number
            const dot = document.createElement("span");
            dot.className = "w-1.5 h-1.5 rounded-full bg-teal-650 mt-1 block absolute bottom-1.5";
            ...
            cell.addEventListener("mouseenter", () => {
                ...
                if (detailsBox) {
                    detailsBox.innerHTML = dayApps.map(app => {
                        ...
                        const ptName = pt ? pt.name : `مريض رقم ${app.patientId}`;
                        return `<strong>المريض:</strong> ${ptName} | <strong>الوقت:</strong> ${app.time} | <strong>السبب:</strong> ${app.reason}`;
                    }).join("<br>");
                    detailsBox.className = "text-[11px] text-teal-700 font-semibold leading-relaxed";
                }
            });
            ...
            cell.addEventListener("mouseleave", () => {
                const detailsBox = document.getElementById("hover-appointment-details");
                if (detailsBox) {
                    detailsBox.innerHTML = "مرر الفأرة فوق أي تاريخ محدد بنقاط زرقاء لعرض تفاصيل موعد زيارة المريض هنا.";
                    detailsBox.className = "text-[11px] text-teal-700 font-medium";
                }
            });
        } else {
            // Standard day
        ...
        if (day === 17) {
            cell.className += "ring-2 ring-teal-600 ring-offset-1 font-bold ";
        }
...
function renderCriticalPatients(patients) {
    ...
    const criticalList = patients.filter(p => p.condition === "حرجة");
    
    if (criticalList.length === 0) {
        listBody.innerHTML = `
            <tr>
                <td colspan="3" class="px-4 py-8 text-center text-gray-400">لا توجد حالات حرجة حالياً</td>
            </tr>
        `;
        return;
    }
...
function renderRecentPatients(patients) {
    ...
    listBody.innerHTML = recent.map(p => `
        <tr class="hover:bg-teal-50/10 transition-colors">
            <td class="px-4 py-3 font-semibold text-gray-800">${p.name}</td>
            <td class="px-4 py-3 text-gray-600 font-medium">${p.age} سنة / ${p.gender}</td>
            <td class="px-4 py-3 text-gray-500 font-medium">${p.diagnosis}</td>
        </tr>
    `).join('');
}
...
function renderOverviewPatients(patients) {
    ...
    const getConditionBadge = (cond) => {
        if (cond === 'حرجة') {
            return `<span class="bg-red-50 text-red-700 px-2.5 py-0.5 rounded-full text-xs font-bold inline-block border border-red-200">حرجة</span>`;
        }
        if (cond === 'تحتاج إلى متابعة') {
            return `<span class="bg-amber-50 text-amber-700 px-2.5 py-0.5 rounded-full text-xs font-bold inline-block border border-amber-200">تحتاج إلى متابعة</span>`;
        }
        return `<span class="bg-emerald-50 text-emerald-700 px-2.5 py-0.5 rounded-full text-xs font-bold inline-block border border-emerald-200">مستقرة</span>`;
    };
...
function showSuccessNotification() {
    ...
    if (!toast) {
        ...
        toast.className = "fixed bottom-5 left-5 bg-teal-600 text-white px-5 py-3 rounded-lg shadow-lg z-50 transition-all duration-300 transform translate-y-10 opacity-0 flex items-center gap-2";
        toast.dir = "rtl";
        toast.innerHTML = `<i class="fa-solid fa-circle-check text-base"></i> <span class="text-sm font-semibold">تمت إضافة الحالة الحرجة بنجاح وتحديث لوحة التحكم!</span>`;
